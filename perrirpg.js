(() => {
  ...
  const itemArtwork = {
    ...
    "potion-weak": "pocion-pequeña.png",
  ...
  function itemVisual(item, detail = false) {
    ...
    if (!artwork && !typeIcon) return "";
    const name = escapeHtml(item?.name || "Objeto");
    return `<span class="item-visual${detail ? " detail-visual" : ""}">${artwork ? `<img class="item-artwork" src="${artwork}" alt="${name}" loading="lazy">` : ""}${typeIcon ? `<img class="item-type-icon" src="${typeIcon}" alt="${escapeHtml(item?.family || item?.kind || item?.slot || "Tipo de objeto")}" title="${escapeHtml(item?.family || item?.kind || item?.slot || "Tipo de objeto")}" loading="lazy">` : ""}</span>`;
  }
  ...
  function renderCatalog() {
    ...
    $("#rpgCatalog").innerHTML = items.map((item) => {
      ...
      return `<button type="button" class="item-card ${state.selectedId === item.id ? "active" : ""}" data-item-id="${escapeHtml(item.id)}"><span class="kind">${escapeHtml(item.setName || item.family || item.kind)}</span><div class="item-card-body">${itemVisual(item)}<span class="item-card-copy"><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(itemEffect(item))}</small></span></div><footer><span>${item.price === null ? "PRECIO PENDIENTE" : `✦ ${item.price} PG`}</span><b>×${quantity}</b></footer></button>`;
