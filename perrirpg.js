(() => {
  ...
  const posePath = (name) => `./assets/perrirpg/${name}.png`;
  ...
  function applyServerState(data) {
    ...
    if (!state.combat && data.combat) state.combat = normalizeSavedCombat(data.combat);
  ...
  function filteredCatalog() {
    ...
    return kind ? state.catalog.filter((item) => item.kind === kind) : state.catalog;
  }
  ...
  function renderCatalog() {
    ...
    $("#rpgCatalog").innerHTML = items.map((item) => {
      ...
      return `<button type="button" class="item-card ${state.selectedId === item.id ? "active" : ""}" data-item-id="${escapeHtml(item.id)}"><span class="kind">${escapeHtml(item.setName || item.family || item.kind)}</span><div><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(itemEffect(item))}</small></div><footer><span>${item.price === null ? "PRECIO PENDIENTE" : `✦ ${item.price} PG`}</span><b>×${quantity}</b></footer></button>`;
  ...
  function renderDetail() {
    ...
    const unavailable = item.price === null || state.pg < Number(item.price || 0) || atMax || state.busy;
    const buttonLabel = item.price === null ? "PRECIO PENDIENTE" : atMax ? `MÁXIMO ${item.maxQuantity}` : state.pg < item.price ? "PG INSUFICIENTES" : `COMPRAR · ✦ ${item.price} PG`;
    $("#rpgDetail").innerHTML = `<span class="detail-kind">${escapeHtml(item.kind)} / ${escapeHtml(item.setName || item.family)}</span><h2>${escapeHtml(item.name)}</h2><p>${escapeHtml(item.effect)}</p><div class="detail-facts"><div><span>EFECTO</span><b>${escapeHtml(itemEffect(item))}</b></div><div><span>POSEES</span><b>${quantity} / ${item.maxQuantity}</b></div><div><span>CONSUMO</span><b>${item.kind === "armor" ? "Permanente" : "Un uso"}</b></div></div><button type="button" class="arcane-button" data-buy-id="${escapeHtml(item.id)}" ${unavailable ? "disabled" : ""}>${buttonLabel}</button>`;
  }
  ...
  async function buyItem(itemId) {
    ...
    renderDetail();
    try {
      const data = await api("publicShopRpgPurchase", { itemId });
      if (!data?.ok) {
        const messages = { max_quantity: "Ya tienes la cantidad máxima.", price_pending: "El precio de esta armadura aún no está fijado.", not_enough_polvoGema: "No tienes suficientes PG." };
        toast(messages[data?.error] || "No se pudo completar la compra.", true);
        return;
      }
      applyServerState(data);
      toast(`${state.catalog.find((item) => item.id === itemId)?.name || "Objeto"} añadido al inventario.`);
  ...
  function renderEquipment() {
    ...
    $("#rpgLoadout").innerHTML = ownedBattleItems.map((item) => {
      ...
      return `<article class="loadout-item"><div><strong>${escapeHtml(item.name)}</strong><small>×${state.inventory[item.id]} · máximo ${item.maxQuantity}</small></div><button type="button" class="${active ? "active" : ""}" data-loadout-id="${escapeHtml(item.id)}" data-enabled="${active ? "false" : "true"}">${active ? "ACTIVO" : "AÑADIR"}</button></article>`;
  ...
  function renderCombatMenu() {
    ...
    if (root) {
      ...
      html = entries.map((item) => `<button class="combat-button" data-action-id="${escapeHtml(item.id)}"><b>×${state.inventory[item.id]}</b>${escapeHtml(item.name)}<small>${escapeHtml(itemEffect(item))}</small></button>`).join("") || '<button class="combat-button" disabled>No tienes objetos preparados</button>';
  ...
  async function performEnemyAction(index) {
    ...
    else if (!spiritual && oak) damage = Math.max(1, round(damage * .65));
  ...
  function finishRound() {
    ...
    renderCombat(); renderEquipment(); saveCombat();
  }

  async function saveCombat() {
    if (!state.sessionToken || !state.combat) return;
    const combat = state.combat;
    api("publicShopRpgSaveCombat", { combat: { round: combat.round, playerHp: combat.player.hp, playerMaxHp: combat.player.maxHp, enemyHp: combat.enemy.hp, enemyMaxHp: combat.enemy.maxHp, undead: combat.player.undead, playerStatuses: combat.player.statuses, enemyStatuses: combat.enemy.statuses, log: combat.log } }).catch(() => {});
  }
  ...
  function showMainTab(tab) {
    ...
    if (tab === "combat") { if (!state.combat) newCombat(); renderCombat(); startAnimation(); }
    else stopAnimation();
  }
  ...
  document.addEventListener("click", (event) => {
    const main = event.target.closest("[data-main-tab]"); if (main) return showMainTab(main.dataset.mainTab);
    ...
    const buy = event.target.closest("[data-buy-id]"); if (buy) return buyItem(buy.dataset.buyId);
  ...
  $("#newCombat").addEventListener("click", newCombat);
