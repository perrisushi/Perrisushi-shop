(() => {
  ...
  function renderDetail() {
    ...
    const purchaseButtons = amounts.map((requested) => {
      ...
      const detail = item.price === null ? "Precio pendiente" : atMax ? `Máximo ${item.maxQuantity}` : state.pg < cost ? `${cost} PG necesarios` : delivered < requested ? `${delivered} uds · ${cost} PG` : `${cost} PG`;
  ...
  async function buyItem(itemId, requestedQuantity = 1) {
    ...
    try {
      ...
      toast(`${state.catalog.find((item) => item.id === itemId)?.name || "Objeto"}: +${purchased} por ${cost} PG${adjusted ? " · ajustado al máximo" : ""}.`);
