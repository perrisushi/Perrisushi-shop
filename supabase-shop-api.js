...
const PERRI_CASILLAS_BOARD_VERSION = 2;
...
function publicPerriCasillasGame(game) {
  ...
  const windowActive = Number.isFinite(windowStartedMs) && Date.now() - windowStartedMs < PERRI_CASILLAS_WINDOW_MS;
  const playsUsed = windowActive ? Math.max(0, Math.min(PERRI_CASILLAS_MAX_GAMES, Number(game.playsInWindow || 0))) : 0;
  return {
...
async function getOrCreatePerriCasillasGame(nick) {
  ...
  if (row) {
    ...
    if (!Number.isFinite(windowStartedMs) || Date.now() - windowStartedMs >= PERRI_CASILLAS_WINDOW_MS) {
...
async function publicShopResetPerriCasillas(sessionToken) {
  ...
  return withNickLock(sessionResult.nick, async () => {
    ...
    if (currentGame.playsInWindow >= PERRI_CASILLAS_MAX_GAMES) {
      const gameForClient = publicPerriCasillasGame(currentGame);
      return {
        ok: false,
    ...
    const nextPlaysInWindow = currentGame.playsInWindow + 1;
    const game = {
      roundId: crypto.randomUUID(),
      cells: createPerriCasillasBoard(),
      playsInWindow: nextPlaysInWindow,
      windowStartedAt: nextPlaysInWindow >= PERRI_CASILLAS_MAX_GAMES || currentGame.playsInWindow === 0
        ? nowIso()
        : currentGame.windowStartedAt
    };
