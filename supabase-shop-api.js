...
const PERRI_CHEST_FRAGMENT_CHANCE = 5;
const PERRI_CHEST_LOGO_CHANCE = 30;
const MINIGAME_CHEST_FRAGMENT_CHANCE = 2;
...
function buildPerriChestExtraReward(profile, logoCatalog) {
  ...
  if (extraRoll < PERRI_CHEST_FRAGMENT_CHANCE) {
    return buildInventoryReward("fragmentoKey", 1);
  }

  if (extraRoll < (PERRI_CHEST_FRAGMENT_CHANCE + PERRI_CHEST_LOGO_CHANCE) && unknownLogos.length > 0) {
...
function buildMinigameChestRewards() {
  ...
  const extraReward = (Math.random() * 100) < MINIGAME_CHEST_FRAGMENT_CHANCE
    ? buildInventoryReward("fragmentoKey", 1)
    : null;
  if (extraReward) {
    rewards.push(extraReward);
  }

  return {
    guaranteedRewards: rewards.filter((reward) => reward?.kind !== "fragmentoKey"),
    extraReward,
    rewards
  };
}
