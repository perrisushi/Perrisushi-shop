...
async function publicShopToggleChatReaction(sessionToken, messageId, reactionKey) {
  ...
  return withNickLock(sessionResult.nick, async () => {
    ...
    CHAT_REACTION_KEYS.forEach((key) => {
      nextReactions[key] = Array.isArray(message.reactions?.[key]) ? message.reactions[key].slice() : [];
    });
    const existingIndex = nextReactions[normalizedReactionKey].indexOf(sessionResult.nick);
    let reacted = false;
    if (existingIndex >= 0) {
      nextReactions[normalizedReactionKey].splice(existingIndex, 1);
      reacted = false;
    } else {
