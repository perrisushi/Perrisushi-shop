...
const SHOP_ITEMS = {
  ...
  boletoSorteo1: { label: "Boleto de Sorteo 1", price: 3500, currencyKey: "pc", currencyLabel: "PC", maxPerUser: 3 }
};
...
const PUBLIC_SHOP_ITEMS = [
  ...
  { key: "boletoSorteo1", label: "Boleto de Sorteo 1", price: 3500, category: "special", currencyKey: "pc", currencyLabel: "PC", effect: "Boleto de sorteo acumulable. Maximo 3 por usuario.", maxPerUser: 3 }
];
...
async function fetchNickRowCaseInsensitive(tableName, nick, options = {}) {
  ...
  const matchingRows = (exactRow ? [exactRow, ...candidateRows] : candidateRows)
    .filter((row) => normalizeNickKey(row?.nick) === normalizedKey);
  const distinctVariants = Array.from(new Set(matchingRows.map((row) => normalizeNick(row?.nick)).filter(Boolean)));
  if (distinctVariants.length > 1) {
    const conflictError = new Error(`nick_case_conflict:${distinctVariants.join("|")}`);
    conflictError.code = "nick_case_conflict";
    conflictError.nickVariants = distinctVariants;
    throw conflictError;
  }
...
async function resolveStoredNick(nick) {
  const normalizedNick = normalizeNick(nick);
  if (!normalizedNick) {
    return "";
  }

  for (const tableName of ["shop_access", "shop_players", "shop_access_requests", "shop_profiles", "shop_inventories", "shop_public_users"]) {
    const row = await fetchNickRowCaseInsensitive(tableName, normalizedNick, { select: "nick" });
    if (row && normalizeNick(row.nick)) {
      return normalizeNick(row.nick);
    }
  }
...
function buildNickCaseConflictResponse(nick, error) {
  return {
    ok: false,
    error: "nick_case_conflict",
    nick: normalizeNick(nick),
    nickVariants: Array.isArray(error?.nickVariants) ? error.nickVariants : []
  };
}
...
async function ensurePlayer(nick) {
  const normalizedNick = await resolveStoredNick(nick);
  if (!normalizedNick) {
    return null;
  }
...
async function playerExists(nick) {
  ...
  const row = await fetchNickRowCaseInsensitive("shop_players", normalizedNick, { select: "nick" });
  return Boolean(row);
}
...
async function hasActiveWebAccess(nick) {
  ...
  const row = await getWebAccessRow(normalizedNick);
...
async function adminCreateAccountActivationLink(nick, expiresAt = "", issuedNote = "") {
  ...
  let canonicalNick = normalizedNick;
  try {
    canonicalNick = await resolveStoredNick(normalizedNick);
    if (!(await playerExists(canonicalNick))) {
      return { ok: false, error: "nick_not_found", nick: canonicalNick };
    }
  } catch (error) {
    if (error?.code === "nick_case_conflict") {
      return buildNickCaseConflictResponse(normalizedNick, error);
    }
    throw error;
  }
...
async function publicShopActivateAccount(token, password) {
  ...
  let canonicalNick = activationLink.nick;
  try {
    canonicalNick = await resolveStoredNick(activationLink.nick);
    if (!(await playerExists(canonicalNick))) {
      return { ok: false, error: "nick_not_found", nick: canonicalNick };
    }
    if (await hasActiveWebAccess(canonicalNick)) {
      return { ok: false, error: "account_already_active", nick: canonicalNick };
    }
  } catch (error) {
    if (error?.code === "nick_case_conflict") {
      return buildNickCaseConflictResponse(activationLink.nick, error);
    }
    throw error;
  }

  return withNickLock(canonicalNick, async () => {
    canonicalNick = await ensurePlayer(canonicalNick);
    const inventory = await getInventory(canonicalNick);
    const profile = await getProfile(canonicalNick);
    ...
    await touchPublicUser(canonicalNick, inventory, profile);
...
async function publicShopRequestAccess(nick, password, requestMessage = "") {
  ...
  try {
    ...
    const existingAccess = await getWebAccessRow(canonicalNick);
    ...
    const pendingRequest = await getPendingAccessRequestByNick(canonicalNick);
    if (pendingRequest) {
      return {
        ok: false,
...
async function adminResolveAccessRequest(requestId, decision, adminNote = "") {
  ...
  if (normalizedDecision === ACCESS_REQUEST_STATUS.approved) {
    ...
    let canonicalNick = accessRequest.nick;
    try {
      canonicalNick = await ensurePlayer(accessRequest.nick);
    } catch (error) {
      if (error?.code === "nick_case_conflict") {
        return buildNickCaseConflictResponse(accessRequest.nick, error);
      }
      throw error;
    }
    await upsertRow("shop_access", {
...
async function publicShopLogin(nick, accessCode) {
  ...
  let accessRow = null;
  try {
    accessRow = await fetchNickRowCaseInsensitive("shop_access", normalizedNick);
  } catch (error) {
    if (error?.code === "nick_case_conflict") {
      return buildNickCaseConflictResponse(normalizedNick, error);
    }
...
async function adminUpsertWebAccess(nick, password, active) {
  ...
  let canonicalNick = normalizedNick;
  try {
    canonicalNick = await ensurePlayer(normalizedNick);
  } catch (error) {
    if (error?.code === "nick_case_conflict") {
      return buildNickCaseConflictResponse(normalizedNick, error);
    }
...
async function adminGetWebAccess(nick) {
  const normalizedNick = normalizeNick(nick);
  if (!normalizedNick) {
    return {
      ok: false,
  ...
  return {
    ...
    updatedAt: String(row.updated_at || "").trim(),
    hasPassword: Boolean(String(row.code_hash || "").trim())
  };
}
...
async function handleShopAction(payload) {
  ...
  switch (action) {
    ...
    case "adminGetWebAccess":
      return adminGetWebAccess(payload.nick);
