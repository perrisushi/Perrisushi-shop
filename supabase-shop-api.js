...
async function getPerriPetStoredState(nick) {
  const row = await fetchRow("shop_operations", {
    filters: {
      operation_id: `eq.${getPerriPetOperationId(nick)}`,
      action: "eq.perripet_state"
    }
  });
  return row?.response?.snapshot && typeof row.response.snapshot === "object"
...
async function publicShopPerriPetState(sessionToken) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) return sessionResult;
  const [snapshot, inventory, rpgInventory] = await Promise.all([
...
async function publicShopPerriPetSave(sessionToken, snapshot) {
  ...
  const previousSnapshot = await getPerriPetStoredState(sessionResult.nick);
  const inventory = await getInventory(sessionResult.nick);
  ...
  await upsertRow("shop_operations", {
    operation_id: getPerriPetOperationId(sessionResult.nick),
    action: "perripet_state",
    nick: sessionResult.nick,
