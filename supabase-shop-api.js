...
const REQUEST_PANEL_OWNER_NICK = "Perrisushi";
...
const LOGO_CATALOG_SEED = [
  ...
  { id: "logo-perriweb1", name: "Perriweb 1", src: "./assets/logos-usuario/logo-perriweb1.png", sortOrder: 150, isDefault: false }
];
...
const CHEST_REWARD_CONFIG = [
  {
    ...
    weight: 60,
    rewards: [
      { label: "1000 Gemas", weight: 50, inventoryDelta: { polvoGema: 1000 } },
      { label: "2000 Gemas", weight: 25, inventoryDelta: { polvoGema: 2000 } },
      { label: "5000 Gemas", weight: 2, inventoryDelta: { polvoGema: 5000 } },
...
const INVENTORY_CHEST_CONFIG = {
  ...
  perriCofresMinijuego: {
    key: "perriCofresMinijuego",
    label: "Perricofre de minijuego",
    lifetimeDays: 30
  }
};

const PERRI_CASILLAS_COST = 200;
...
const PERRIPET_RPG_ITEM_IDS = {
  mageRing: "device-mage-ring",
  arcaneStaff: "device-arcane-staff",
  metalShield: "shield-metal",
  oakShield: "shield-oak",
  royalSword: "sword-royal",
  shiningSword: "sword-shining",
  goldenSpear: "projectile-golden-spear",
  worldbreaker: "greatsword-worldbreaker",
  executionerGreatsword: "greatsword-executioner",
  shuriken: "projectile-shuriken",
  smallRedPotion: "potion-weak",
  mediumRedPotion: "potion-standard",
  largeRedPotion: "potion-potent",
  armorPotion: "potion-armor",
...
const PERRIRPG_DIRECT_SHOP_ITEM_IDS = new Set([
  ...
  "greatsword-rusted",
  "device-broken-wand",
  "projectile-wood-arrow",
  "shield-broken",
  "potion-weak"
]);
const PERRIPET_ITEM_IDS = [
  "food-normal",
  "food-prime",
  "egg-green",
  "capsule-g",
...
const ACHIEVEMENT_CATALOG = [
  { id: "me_gusta_el_marisco", name: "Me gusta el marisco", description: "Derrota a Rey Cangrosio IV en PerriRPG.", icon: "🦀", title: "Me gusta el marisco", metric: "rpgBossWins", target: 1 }
];
const ACHIEVEMENT_ID_SET = new Set(ACHIEVEMENT_CATALOG.map((entry) => entry.id));
...
function getAppDayKey(date = new Date()) {
...
function getDailyPassRewardDefinition(dayNumber) {
  const normalizedDayNumber = Math.max(1, Math.floor(Number(dayNumber) || 0));
  return DAILY_PASS_REWARDS.find((entry) => entry.day === normalizedDayNumber) || null;
}

function mapDailyPassRewardItemForClient(item) {
  const normalizedType = String(item?.type || "").trim();
  const qty = Math.max(1, Math.floor(Number(item?.qty || 1)));
  switch (normalizedType) {
    case "perriCofres":
      return { type: "perriCofres", qty, label: "Perricofre", icon: "./assets/perricofre1-cerrado.png" };
    case "perriCofresMinijuego":
      return { type: "perriCofresMinijuego", qty, label: "Perricofre de minijuego", icon: "./assets/perricofres-minijuego-cerrado.png" };
    case "duelo":
      return { type: "duelo", qty, label: "Ticket Duelo", icon: "./assets/duelo.png" };
    case "monedas":
      return { type: "monedas", qty, label: "Monedas", icon: "./assets/moneda.png" };
    case "polvoGema":
    ...
    default:
      return { type: normalizedType || "unknown", qty, label: normalizedType || "Premio", icon: "" };
  }
}
...
function getInventoryChestConfig(chestType) {
  const normalizedChestType = String(chestType || "").trim();
  if (!normalizedChestType) {
    return INVENTORY_CHEST_CONFIG.perriCofres;
  }
  return INVENTORY_CHEST_CONFIG[normalizedChestType] || null;
}

function getTrackedInventoryChestKeys() {
  return Object.keys(INVENTORY_CHEST_CONFIG);
}
...
function rowToProfile(nick, row, knownLogoIds = FALLBACK_LOGO_ID_SET) {
  ...
  return {
    ...
    selectedTitleId: String(row.selected_title_id || row.selectedTitleId || "").trim()
  };
}
...
function profileToRow(nick, profile, knownLogoIds = FALLBACK_LOGO_ID_SET) {
  ...
  return {
    ...
    selected_title_id: String(normalizedProfile.selectedTitleId || "").trim(),
    updated_at: nowIso()
  };
}
...
async function loadInventoryWithoutChestSync(nick) {
  const normalizedNick = normalizeNick(nick);
  if (!normalizedNick) {
...
async function persistInventoryRow(nick, inventory) {
...
async function expireInventoryChestRows(nick) {
  ...
  try {
    await updateRows("shop_inventory_chests", {
      status: INVENTORY_CHEST_STATUS.expired,
      updated_at: nowIso()
    }, {
      nick: `eq.${normalizedNick}`,
      status: `eq.${INVENTORY_CHEST_STATUS.available}`,
...
async function countAvailableInventoryChests(nick) {
  const normalizedNick = normalizeNick(nick);
  const counts = {
    perriCofres: 0,
    perriCofresMinijuego: 0
  };
  if (!normalizedNick) {
    return counts;
  }
  ...
  try {
    await expireInventoryChestRows(normalizedNick);
    const rows = await fetchRows("shop_inventory_chests", {
      select: "chest_type",
      filters: {
        nick: `eq.${normalizedNick}`,
        status: `eq.${INVENTORY_CHEST_STATUS.available}`,
      ...
      orderBy: "expires_at.asc,obtained_at.asc,chest_id.asc",
      limit: 5000
    });
    rows.forEach((row) => {
      const config = getInventoryChestConfig(row?.chest_type);
      if (config) {
        counts[config.key] = toNumber(counts[config.key]) + 1;
      }
    });
    return counts;
  } catch (error) {
...
async function syncInventoryChestState(nick, inventory = null) {
  const normalizedNick = normalizeNick(nick);
  if (!normalizedNick) {
    return {
  ...
  const baseInventory = inventory ? rowToInventory(inventoryToRow(normalizedNick, inventory)) : await loadInventoryWithoutChestSync(normalizedNick);
  const chestCounts = await countAvailableInventoryChests(normalizedNick);
  if (!chestCounts) {
    return {
  ...
  const syncedInventory = {
    ...baseInventory,
    perriCofres: toNumber(chestCounts.perriCofres),
    perriCofresMinijuego: toNumber(chestCounts.perriCofresMinijuego)
  };
  const needsResave = getTrackedInventoryChestKeys().some((key) => toNumber(baseInventory[key]) !== toNumber(syncedInventory[key]));
  if (needsResave) {
    await persistInventoryRow(normalizedNick, syncedInventory);
  }
...
async function issueInventoryChestRows(nick, chestType, amount) {
  const normalizedNick = normalizeNick(nick);
  const config = getInventoryChestConfig(chestType);
  const normalizedAmount = Math.max(0, Math.floor(Number(amount) || 0));
  if (!normalizedNick || !config || normalizedAmount <= 0) {
    return false;
  }

  try {
    const obtainedAtIso = nowIso();
    const expiresAtIso = new Date(new Date(obtainedAtIso).getTime() + (config.lifetimeDays * 24 * 60 * 60 * 1000)).toISOString();
    for (let index = 0; index < normalizedAmount; index += 1) {
      await insertRow("shop_inventory_chests", {
        chest_id: crypto.randomUUID(),
        nick: normalizedNick,
        chest_type: config.key,
        status: INVENTORY_CHEST_STATUS.available,
        obtained_at: obtainedAtIso,
        expires_at: expiresAtIso,
        updated_at: obtainedAtIso
      }, { returning: "minimal" });
    }
    return true;
  } catch (error) {
...
async function revokeInventoryChestRows(nick, chestType, amount) {
  const normalizedNick = normalizeNick(nick);
  const config = getInventoryChestConfig(chestType);
  const normalizedAmount = Math.max(0, Math.floor(Number(amount) || 0));
  if (!normalizedNick || !config || normalizedAmount <= 0) {
  ...
  try {
    ...
    for (const row of rows) {
      await updateRows("shop_inventory_chests", {
        status: INVENTORY_CHEST_STATUS.revoked,
        updated_at: nowIso()
      }, {
        chest_id: `eq.${row.chest_id}`,
        status: `eq.${INVENTORY_CHEST_STATUS.available}`
      }, { returning: "minimal" });
    }
    return true;
...
async function consumeNextInventoryChestRow(nick, chestType) {
  const normalizedNick = normalizeNick(nick);
  const config = getInventoryChestConfig(chestType);
  if (!normalizedNick || !config) {
    return {
      tracked: false,
      consumed: false
    };
  ...
  try {
    await expireInventoryChestRows(normalizedNick);
    const nextRow = await fetchRow("shop_inventory_chests", {
      select: "chest_id",
      filters: {
        nick: `eq.${normalizedNick}`,
        chest_type: `eq.${config.key}`,
    ...
    const updatedRows = await updateRows("shop_inventory_chests", {
      status: INVENTORY_CHEST_STATUS.opened,
      opened_at: nowIso(),
      updated_at: nowIso()
    }, {
      chest_id: `eq.${nextRow.chest_id}`,
      status: `eq.${INVENTORY_CHEST_STATUS.available}`
    }, { returning: "representation" });

    return {
      tracked: true,
      consumed: Array.isArray(updatedRows) && updatedRows.length > 0
    };
  } catch (error) {
    return {
      tracked: false,
...
async function getInventory(nick) {
  ...
  const inventory = await loadInventoryWithoutChestSync(normalizedNick);
  const syncResult = await syncInventoryChestState(normalizedNick, inventory);
  return syncResult.inventory;
}

async function saveInventory(nick, inventory) {
  const normalizedNick = await ensurePlayer(nick);
  const targetInventory = rowToInventory(inventoryToRow(normalizedNick, inventory));
  const currentInventory = (await syncInventoryChestState(normalizedNick, await loadInventoryWithoutChestSync(normalizedNick))).inventory;

  let trackingWorked = false;
  for (const chestKey of getTrackedInventoryChestKeys()) {
    const delta = toNumber(targetInventory[chestKey]) - toNumber(currentInventory[chestKey]);
    if (delta > 0) {
      trackingWorked = await issueInventoryChestRows(normalizedNick, chestKey, delta) || trackingWorked;
    } else if (delta < 0) {
      trackingWorked = await revokeInventoryChestRows(normalizedNick, chestKey, Math.abs(delta)) || trackingWorked;
    }
  }

  if (trackingWorked) {
    const finalSync = await syncInventoryChestState(normalizedNick, {
      ...targetInventory,
      perriCofres: currentInventory.perriCofres,
      perriCofresMinijuego: currentInventory.perriCofresMinijuego
    });
    return persistInventoryRow(normalizedNick, finalSync.inventory);
  }

  return persistInventoryRow(normalizedNick, targetInventory);
}
...
async function touchPublicUser(nick, inventory, profile) {
  ...
  await upsertRow("shop_public_users", {
    ...
    selected_title_id: String(profile.selectedTitleId || "").trim(),
    updated_at: nowIso()
...
function mapChatMessageRow(row) {
  ...
  const message = normalizeChatMessageText(row.message);
  if (!id || !nick || !message) {
    return null;
  }
  const rawReactions = row.reactions && typeof row.reactions === "object" && !Array.isArray(row.reactions)
    ? row.reactions
    : {};
  const normalizedReactions = {};
  CHAT_REACTION_KEYS.forEach((reactionKey) => {
    const source = Array.isArray(rawReactions[reactionKey])
      ? rawReactions[reactionKey]
      : [];
    normalizedReactions[reactionKey] = source
      .map((entry) => normalizeNick(entry))
      .filter(Boolean);
  });
  if ((!normalizedReactions.like || !normalizedReactions.like.length) && Array.isArray(row.liked_by_nicks)) {
    normalizedReactions.like = row.liked_by_nicks.map((entry) => normalizeNick(entry)).filter(Boolean);
  }
  return {
    id,
    nick,
    logoId: normalizePublicLogoId(row.logo_id) || DEFAULT_LOGO_ID,
    message,
    reactions: normalizedReactions,
    replyToMessageId: String(row.reply_to_message_id || "").trim(),
    createdAt: String(row.created_at || "").trim()
  };
}

function mapChatMessageForClient(message, viewerNick = "") {
  const normalizedViewerNick = normalizeNick(viewerNick);
  const reactions = CHAT_REACTION_KEYS.map((reactionKey) => {
    const reactedBy = Array.isArray(message?.reactions?.[reactionKey]) ? message.reactions[reactionKey] : [];
    return {
      key: reactionKey,
      count: reactedBy.length,
      reactedByMe: Boolean(normalizedViewerNick && reactedBy.includes(normalizedViewerNick))
    };
  }).filter((entry) => entry.count > 0);
  return {
    id: String(message?.id || "").trim(),
    nick: normalizeNick(message?.nick),
    logoId: normalizePublicLogoId(message?.logoId) || DEFAULT_LOGO_ID,
    message: normalizeChatMessageText(message?.message),
...
async function listChatMessages(limit = CHAT_MESSAGE_LIST_LIMIT, viewerNick = "") {
  ...
  return rows
    ...
    .filter(Boolean)
    .reverse()
    .map((entry) => mapChatMessageForClient(entry, viewerNick));
}

async function touchChatPresence(nick, active = true) {
  const normalizedNick = normalizeNick(nick);
  if (!normalizedNick) {
    return;
  }
  if (!active) {
    await deleteRows("shop_chat_presence", {
      nick: `eq.${normalizedNick}`
    }, { returning: "minimal" });
    return;
  }

  await upsertRow("shop_chat_presence", {
    nick: normalizedNick,
    last_seen_at: nowIso(),
    updated_at: nowIso()
  }, "nick", { returning: "minimal" });
...
async function countActiveChatUsers() {
  const cutoffIso = new Date(Date.now() - CHAT_PRESENCE_TTL_MS).toISOString();
  const rows = await fetchRows("shop_chat_presence", {
    select: "nick",
    filters: {
...
async function publicShopListChatMessages(sessionToken) {
  ...
  await touchChatPresence(sessionResult.nick, true);
...
async function publicShopSendChatMessage(sessionToken, message, replyToMessageId) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) {
    return sessionResult;
  }
  ...
  return withNickLock(sessionResult.nick, async () => {
    const text = normalizeChatMessageText(message);
    if (!text) {
      return {
        ok: false,
        error: "missing_chat_message",
        nick: sessionResult.nick,
        messages: await listChatMessages(CHAT_MESSAGE_LIST_LIMIT, sessionResult.nick)
      };
    }
    ...
    if (normalizedReplyToMessageId) {
      const replyRow = await fetchRow("shop_chat_messages", {
        select: "id",
        filters: {
          id: `eq.${normalizedReplyToMessageId}`
        }
      });
      if (!replyRow) {
        return {
        ok: false,
        error: "invalid_chat_reply",
        nick: sessionResult.nick,
        messages: await listChatMessages(CHAT_MESSAGE_LIST_LIMIT, sessionResult.nick)
      };
    }
    ...
    const latestOwnMessage = await fetchRow("shop_chat_messages", {
      select: "created_at",
      filters: {
    ...
    const latestOwnMessageAt = new Date(String(latestOwnMessage?.created_at || 0)).getTime();
    if (latestOwnMessageAt && (Date.now() - latestOwnMessageAt) < CHAT_MESSAGE_COOLDOWN_MS) {
      return {
        ok: false,
        error: "chat_cooldown",
        retryAfterMs: Math.max(1000, CHAT_MESSAGE_COOLDOWN_MS - (Date.now() - latestOwnMessageAt)),
        nick: sessionResult.nick,
        messages: await listChatMessages(CHAT_MESSAGE_LIST_LIMIT, sessionResult.nick)
      };
    }
    ...
    const insertedRows = await insertRow("shop_chat_messages", {
      nick: sessionResult.nick,
      logo_id: normalizePublicLogoId(profile.selectedLogoId) || DEFAULT_LOGO_ID,
      message: text,
      reply_to_message_id: normalizedReplyToMessageId || null,
      created_at: nowIso(),
      updated_at: nowIso()
    });
    const insertedMessage = Array.isArray(insertedRows) ? insertedRows[0] : insertedRows;
    const mappedMessage = mapChatMessageRow(insertedMessage);

    return {
      ok: true,
      nick: sessionResult.nick,
      message: mapChatMessageForClient(mappedMessage, sessionResult.nick),
      messages: await listChatMessages(CHAT_MESSAGE_LIST_LIMIT, sessionResult.nick),
      activeChatUsers: await countActiveChatUsers()
    };
  });
...
async function publicShopToggleChatLike(sessionToken, messageId) {
  return publicShopToggleChatReaction(sessionToken, messageId, "like");
}

async function publicShopToggleChatReaction(sessionToken, messageId, reactionKey) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) {
    return sessionResult;
  }
  ...
  return withNickLock(sessionResult.nick, async () => {
    ...
    const normalizedReactionKey = String(reactionKey || "").trim().toLowerCase();
    if (!normalizedMessageId) {
        return {
          ok: false,
          error: "missing_chat_message_id",
          nick: sessionResult.nick,
          messages: await listChatMessages(CHAT_MESSAGE_LIST_LIMIT, sessionResult.nick),
        activeChatUsers: await countActiveChatUsers()
      };
    }
    if (!CHAT_REACTION_KEYS.includes(normalizedReactionKey)) {
      return {
        ok: false,
        error: "invalid_chat_reaction",
        nick: sessionResult.nick,
        messages: await listChatMessages(CHAT_MESSAGE_LIST_LIMIT, sessionResult.nick),
        activeChatUsers: await countActiveChatUsers()
      };
    ...
    const row = await fetchRow("shop_chat_messages", {
      select: "id,nick,logo_id,message,liked_by_nicks,reactions,reply_to_message_id,created_at",
      filters: {
    ...
    const nextReactions = {};
    CHAT_REACTION_KEYS.forEach((key) => {
      const source = Array.isArray(message.reactions?.[key]) ? message.reactions[key].slice() : [];
      nextReactions[key] = source.filter((entry) => entry !== sessionResult.nick);
    });
    const hadSameReaction = Array.isArray(message.reactions?.[normalizedReactionKey])
      ? message.reactions[normalizedReactionKey].includes(sessionResult.nick)
      : false;
    let reacted = false;
    if (!hadSameReaction) {
      nextReactions[normalizedReactionKey].push(sessionResult.nick);
      reacted = true;
    }

    await updateRows("shop_chat_messages", {
      liked_by_nicks: normalizedReactionKey === "like" ? nextReactions.like : (Array.isArray(message.reactions?.like) ? message.reactions.like : []),
...
async function publicShopLeaveChat(sessionToken) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) {
    return sessionResult;
  }

  await touchChatPresence(sessionResult.nick, false);
  return {
    ok: true,
    nick: sessionResult.nick
  };
...
function normalizeDailyPassClaimedDays(value) {
  return Math.max(0, Math.min(DAILY_PASS_REWARDS.length, Math.floor(Number(value) || 0)));
}
...
function buildDailyPassState(row) {
  ...
  const completed = claimedDays >= DAILY_PASS_REWARDS.length;
  ...
  return {
    ...
    canClaimToday,
    currentRewardDay,
    rewards: DAILY_PASS_REWARDS.map((entry) => ({
      day: entry.day,
      items: entry.items.map(mapDailyPassRewardItemForClient)
    }))
  };
}

async function getDailyPassState(nick) {
  const normalizedNick = normalizeNick(nick);
  if (!normalizedNick) {
    return buildDailyPassState(null);
  }
  ...
  return buildDailyPassState(accessRow);
}

function buildDailyPassInventoryDelta(items = []) {
...
function formatDailyPassRewardSummary(items = []) {
  return (Array.isArray(items) ? items : [])
    .map((item) => {
      const mapped = mapDailyPassRewardItemForClient(item);
      return `${mapped.qty} ${mapped.label}`;
    })
    .join(" + ");
}

function normalizeFreePlayDay(value) {
  return String(value || "").trim();
}
...
async function getDailyFreePlayState(nick) {
  const normalizedNick = normalizeNick(nick);
  if (!normalizedNick) {
    return mapDailyFreePlayState(null);
  }
  ...
  return mapDailyFreePlayState(accessRow);
}

async function consumeDailyFreePlay(nick, gameKey) {
  const normalizedNick = normalizeNick(nick);
  ...
  if (!normalizedNick || !patch) {
    return {
      consumed: false,
      state: mapDailyFreePlayState(null)
    };
  }

  await updateRows("shop_access", patch, {
    nick: `eq.${normalizedNick}`
  }, { returning: "minimal" });

  return {
    consumed: true,
    state: await getDailyFreePlayState(normalizedNick)
  };
}
...
async function buildAchievementSystem(nick, inventory, profile) {
  const normalizedNick = normalizeNick(nick);
  const [rows, rpgCombatState] = await Promise.all([
    fetchRows("shop_activity", {
      select: "game,result,details",
      filters: { nick: `eq.${normalizedNick}` },
      orderBy: "created_at.desc",
      limit: 1000
    }),
    fetchRow("shop_rpg_combat_state", {
      select: "enemy_hp",
      filters: { nick: `eq.${normalizedNick}` }
    })
  ]);
  const normalizedRows = rows.map((row) => ({
    game: String(row.game || "").trim().toLowerCase(),
    result: String(row.result || "").trim().toLowerCase(),
    details: String(row.details || "").trim().toLowerCase()
  }));
  const metrics = {
    activities: normalizedRows.length,
    pc: toNumber(inventory.pc),
    dust: toNumber(inventory.polvoGema),
    purchases: normalizedRows.filter((row) => row.game === "tienda" && row.result.includes("compra")).length,
    chests: normalizedRows.filter((row) => row.game.includes("cofre") || row.game.includes("chest")).length,
    duelWins: normalizedRows.filter((row) => row.game.includes("duelo") && (row.result.includes("victoria") || row.result.includes("ganador") || row.details.includes("victoria"))).length,
    logos: Array.isArray(profile.ownedLogoIds) ? profile.ownedLogoIds.length : 0,
    legendaryGear: toNumber(inventory.espadaLegendaria) + toNumber(inventory.escudoLegendario) > 0 ? 1 : 0,
    rpgBossWins: normalizedRows.some((row) => row.game === "perrirpg" && row.result.includes("rey cangrosio iv derrotado")) || (rpgCombatState && Number(rpgCombatState.enemy_hp) <= 0) ? 1 : 0
  };
  const storedAchievements = Array.isArray(profile.achievements) ? profile.achievements.map((entry) => String(entry || "").trim()).filter(Boolean) : [];
  const unlockedIds = new Set(storedAchievements.filter((entry) => ACHIEVEMENT_ID_SET.has(entry)));
  const newlyUnlockedIds = [];
  ACHIEVEMENT_CATALOG.forEach((achievement) => {
    if (toNumber(metrics[achievement.metric]) >= achievement.target && !unlockedIds.has(achievement.id)) {
      unlockedIds.add(achievement.id);
      newlyUnlockedIds.push(achievement.id);
    }
  });
  const selectedTitleId = unlockedIds.has(String(profile.selectedTitleId || "")) ? String(profile.selectedTitleId || "") : "";
  const legacyAchievements = storedAchievements.filter((entry) => !ACHIEVEMENT_ID_SET.has(entry));
  const nextProfile = {
    ...profile,
    achievements: [...legacyAchievements, ...ACHIEVEMENT_CATALOG.map((entry) => entry.id).filter((id) => unlockedIds.has(id))],
    selectedTitleId
  };
  return {
    profile: nextProfile,
    changed: JSON.stringify(nextProfile.achievements) !== JSON.stringify(storedAchievements) || selectedTitleId !== String(profile.selectedTitleId || ""),
    achievementSystem: {
      catalog: ACHIEVEMENT_CATALOG.map(({ metric, ...entry }) => ({
        ...entry,
        progress: Math.min(entry.target, toNumber(metrics[metric])),
...
async function syncAchievementSystem(nick, inventory, profile) {
  const result = await buildAchievementSystem(nick, inventory, profile);
  if (result.changed) {
    result.profile = await saveProfile(nick, result.profile);
  }
  return result;
}
...
function buildCardRewardDelta(reward) {
  ...
  if (normalizedReward === "+2 Monedas") {
    return { monedas: 2 };
  }
  if (normalizedReward === "+3 Monedas") {
    return { monedas: 3 };
  }
  if (normalizedReward === "+1 Perricofre de minijuego") {
    return { perriCofresMinijuego: 1 };
  }
...
function buildInventoryReward(kind, quantity) {
  ...
  if (kind === "polvoGema") {
    return {
      kind,
      label: `${normalizedQuantity} Gemas`,
      quantity: normalizedQuantity,
  ...
  if (kind === "perricita") {
    return {
      kind,
      label: formatRewardLabel(normalizedQuantity, "Perricita"),
      quantity: normalizedQuantity,
      inventoryDelta: { perricita: normalizedQuantity }
    };
  }
...
function buildPerriChestExtraReward(profile, logoCatalog) {
  ...
  if (extraRoll < PERRI_CHEST_LOGO_CHANCE && unknownLogos.length > 0) {
...
function mapAccessRequestRow(row) {
  ...
  return {
    ...
    nick: normalizeNick(row.nick),
    status: String(row.status || "").trim() || ACCESS_REQUEST_STATUS.pending,
    requestMessage: String(row.request_message || "").trim(),
...
async function buildShopState(nick) {
  ...
  let profile = await getProfile(nick);
  const achievementResult = await syncAchievementSystem(nick, syncResult.inventory, profile);
  ...
  const [
    ...
    getDailyFreePlayState(nick),
    getDailyPassState(nick)
  ]);
...
async function buildProfileState(nick) {
  ...
  let profile = await getProfile(normalizedNick);
  const achievementResult = await syncAchievementSystem(normalizedNick, inventory, profile);
  profile = achievementResult.profile;
  const logoCatalog = await listLogoCatalog();
  await touchPublicUser(normalizedNick, inventory, profile);

  const [activityHistory, notifications, unreadNotificationCount, dailyFreePlays, dailyPass] = await Promise.all([
    listActivityForNick(normalizedNick),
    listNotificationsForNick(normalizedNick),
    countUnreadNotificationsForNick(normalizedNick),
    getDailyFreePlayState(normalizedNick),
    getDailyPassState(normalizedNick)
  ]);
...
async function getWebAccessRow(nick) {
  const normalizedNick = normalizeNick(nick);
  if (!normalizedNick) {
    return null;
  }
  return fetchNickRowCaseInsensitive("shop_access", normalizedNick);
}

async function getPendingAccessRequestByNick(nick) {
  const normalizedNick = normalizeNick(nick);
  if (!normalizedNick) {
    return null;
  }
  const row = await fetchNickRowCaseInsensitive("shop_access_requests", normalizedNick, {
    filters: {
      status: `eq.${ACCESS_REQUEST_STATUS.pending}`
    },
...
async function getAccessRequestById(requestId) {
  ...
  const row = await fetchRow("shop_access_requests", {
    filters: {
      request_id: `eq.${normalizedRequestId}`
    }
  });
  return mapAccessRequestRow(row);
}
...
async function publicShopRequestAccess(nick, password, requestMessage = "") {
  const normalizedNick = normalizeNick(nick);
  const normalizedPassword = String(password || "").trim();
  ...
  if (await hasActiveWebAccess(normalizedNick)) {
    return { ok: false, error: "account_already_active", nick: normalizedNick };
  }

  const existingAccess = await getWebAccessRow(normalizedNick);
  if (existingAccess && String(existingAccess.code_hash || "").trim()) {
    return {
      ok: false,
      error: existingAccess.active === false ? "access_exists_inactive" : "access_already_exists",
      nick: normalizedNick
    };
  }

  const pendingRequest = await getPendingAccessRequestByNick(normalizedNick);
  if (pendingRequest) {
    return {
      ok: false,
  ...
  const row = {
    ...
    nick: normalizedNick,
    password_hash: sha256(normalizedPassword),
  ...
  await insertRow("shop_access_requests", row, { returning: "minimal" });
  return {
    ok: true,
    nick: normalizedNick,
    accessRequest: mapAccessRequestRow(row)
  };
}

async function adminListAccessRequests(limit = 50, status = "") {
  const normalizedStatus = String(status || "").trim().toLowerCase();
  const filters = {};
  if (normalizedStatus && Object.values(ACCESS_REQUEST_STATUS).includes(normalizedStatus)) {
    filters.status = `eq.${normalizedStatus}`;
  }

  const rows = await fetchRows("shop_access_requests", {
    select: "*",
    filters,
    orderBy: "requested_at.desc",
    limit: Math.max(1, Math.min(200, Math.floor(Number(limit || 50))))
  ...
  return {
    ok: true,
    accessRequests: rows.map((row) => mapAccessRequestRow(row)).filter(Boolean)
  };
}

async function adminResolveAccessRequest(requestId, decision, adminNote = "") {
  const normalizedRequestId = String(requestId || "").trim();
  ...
  if (normalizedDecision !== ACCESS_REQUEST_STATUS.approved && normalizedDecision !== ACCESS_REQUEST_STATUS.rejected) {
    return { ok: false, error: "invalid_decision" };
  }

  const accessRequest = await getAccessRequestById(normalizedRequestId);
  if (!accessRequest) {
    return { ok: false, error: "request_not_found" };
  }
  if (accessRequest.status !== ACCESS_REQUEST_STATUS.pending) {
    return {
      ok: false,
  ...
  if (normalizedDecision === ACCESS_REQUEST_STATUS.approved) {
    const requestRow = await fetchRow("shop_access_requests", {
      filters: {
        request_id: `eq.${normalizedRequestId}`
      }
    });
    const passwordHash = String(requestRow?.password_hash || "").trim();
    if (!passwordHash) {
      return { ok: false, error: "missing_password_hash", accessRequest };
    }

    await ensurePlayer(accessRequest.nick);
    await upsertRow("shop_access", {
      nick: accessRequest.nick,
  ...
  await updateRows("shop_access_requests", {
  ...
  const resolvedRequest = await getAccessRequestById(normalizedRequestId);
  return {
    ok: true,
    decision: normalizedDecision,
    accessRequest: resolvedRequest
  };
}

async function authorizeAccessRequestPanel(sessionToken, panelKey) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) {
    return sessionResult;
  }

  if (normalizeNick(sessionResult.nick) !== normalizeNick(REQUEST_PANEL_OWNER_NICK)) {
    return {
      ok: false,
      error: "forbidden_panel_user",
      nick: sessionResult.nick
    };
  }

  if (!REQUEST_PANEL_SECRET) {
    return {
      ok: false,
  ...
  if (String(panelKey || "").trim() !== REQUEST_PANEL_SECRET) {
    return {
      ok: false,
      error: "invalid_panel_key",
      nick: sessionResult.nick
    };
  }
...
async function publicShopListAccessRequestsPanel(sessionToken, panelKey, status = "", limit = 50) {
  const authResult = await authorizeAccessRequestPanel(sessionToken, panelKey);
  if (!authResult.ok) {
    return authResult;
  }
  return adminListAccessRequests(limit, status);
}

async function publicShopResolveAccessRequestPanel(sessionToken, panelKey, requestId, decision, adminNote = "") {
  const authResult = await authorizeAccessRequestPanel(sessionToken, panelKey);
  if (!authResult.ok) {
    return authResult;
  }
...
async function listPublicShopUsers() {
  ...
  const profiles = await fetchRows("shop_profiles", {
    select: "nick,selected_logo_id,karma,achievements,selected_title_id"
  });
  ...
  return accessRows
    .map((row) => {
      ...
      return {
        ...
        achievements: profileRow && Array.isArray(profileRow.achievements)
          ? profileRow.achievements.map((id) => ACHIEVEMENT_CATALOG.find((entry) => entry.id === id)?.name || id)
          : [],
        title: ACHIEVEMENT_CATALOG.find((entry) => entry.id === String(profileRow?.selected_title_id || ""))?.title || "",
    ...
    .filter(Boolean)
    .sort((left, right) => (right.pc - left.pc) || left.nick.localeCompare(right.nick, "es", { sensitivity: "base" }));
}
...
async function publicShopGetProfile(sessionToken) {
  ...
  return {
    ...
    dailyFreePlays: state.dailyFreePlays,
    dailyPass: state.dailyPass
  };
}

async function buildRpgState(nick) {
  const normalizedNick = normalizeNick(nick);
  await supabaseRequest("rpc/shop_rpg_ensure_starter_set", {
    method: "POST",
    body: JSON.stringify({ p_nick: normalizedNick })
  });
  const [catalogRows, inventoryRows, equipmentRows, loadoutRows, combatState, shopInventory] = await Promise.all([
    fetchRows("shop_rpg_catalog", {
      filters: { enabled: "eq.true" },
      orderBy: "sort_order.asc"
    }),
    fetchRows("shop_rpg_inventory", {
      filters: { nick: `eq.${normalizedNick}`, quantity: "gt.0" },
      orderBy: "item_id.asc"
    }),
    fetchRows("shop_rpg_equipment", {
      filters: { nick: `eq.${normalizedNick}` },
      orderBy: "slot.asc"
    }),
    fetchRows("shop_rpg_loadout", {
      filters: { nick: `eq.${normalizedNick}`, enabled: "eq.true" },
      orderBy: "item_id.asc"
    }),
    fetchRow("shop_rpg_combat_state", {
      filters: { nick: `eq.${normalizedNick}` }
  ...
  return {
    ...
    catalog: catalogRows.map((row) => ({
      ...
      price: row.price_pg === null || row.price_pg === undefined ? null : toNumber(row.price_pg),
      maxQuantity: Math.max(1, toNumber(row.max_quantity)),
    ...
    combat: combatState ? {
      round: Math.max(1, toNumber(combatState.round_number)),
      playerHp: Number(combatState.player_hp || 0),
      playerMaxHp: Number(combatState.player_max_hp || 0),
      enemyHp: Number(combatState.enemy_hp || 5000),
      enemyMaxHp: Number(combatState.enemy_max_hp || 5000),
      undead: Boolean(combatState.is_undead),
      playerStatuses: Array.isArray(combatState.player_statuses) ? combatState.player_statuses : [],
      enemyStatuses: Array.isArray(combatState.enemy_statuses) ? combatState.enemy_statuses : [],
      log: Array.isArray(combatState.combat_log) ? combatState.combat_log : []
    } : null
  };
}

async function publicShopRpgState(sessionToken) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) return sessionResult;
  return { ok: true, ...(await buildRpgState(sessionResult.nick)) };
}

function getPerriPetOperationId(nick) {
  return `perripet-state-${sha256(normalizeNickKey(nick)).slice(0, 32)}`;
}

async function getPerriPetStoredState(nick) {
  const normalizedNick = normalizeNick(nick);
  const stateRow = await fetchRow("shop_perripet_states", {
    filters: {
      nick: `eq.${normalizedNick}`
    }
  });
  if (stateRow) {
    return {
      legacy: false,
      snapshot: {
        version: Math.max(1, toNumber(stateRow.version) || 1),
        pet: stateRow.pet_state && typeof stateRow.pet_state === "object" ? stateRow.pet_state : {},
        farm: stateRow.farm_state && typeof stateRow.farm_state === "object" ? stateRow.farm_state : {},
        encounters: Array.isArray(stateRow.encounters_state) ? stateRow.encounters_state : [],
        savedAt: new Date(String(stateRow.updated_at || 0)).getTime() || 0
      }
    };
  }

  const legacyRow = await fetchRow("shop_operations", {
    filters: {
      operation_id: `eq.${getPerriPetOperationId(normalizedNick)}`,
      action: "eq.perripet_state"
    }
  });
  return {
    legacy: Boolean(legacyRow?.response?.snapshot),
    snapshot: legacyRow?.response?.snapshot && typeof legacyRow.response.snapshot === "object"
      ? legacyRow.response.snapshot
      : null
  };
}

function emptyPerriPetInventory() {
  return {
    ...Object.fromEntries(PERRIPET_ITEM_IDS.map((itemId) => [itemId, 0])),
    "watering-can-normal": 1
  };
}

function perriPetInventoryFromSnapshot(snapshot) {
  const pet = snapshot?.pet && typeof snapshot.pet === "object" ? snapshot.pet : {};
  const farm = snapshot?.farm && typeof snapshot.farm === "object" ? snapshot.farm : {};
  const loot = pet.lootInventory && typeof pet.lootInventory === "object" ? pet.lootInventory : {};
  return {
    ...emptyPerriPetInventory(),
    "food-normal": toNumber(pet.normalFood),
    "food-prime": toNumber(pet.primeFood),
    "egg-green": toNumber(pet.greenEggs),
    "capsule-g": Array.isArray(pet.capsules) ? pet.capsules.length : 0,
    "seed-green": toNumber(farm.seeds),
    "seed-red": toNumber(farm.redSeeds),
    "fruit-purple": toNumber(farm.purpleFruits),
    "fruit-yellow": toNumber(farm.yellowFruits),
    "plot-unlock": toNumber(farm.plotUnlockItems),
    "watering-can-normal": toNumber(farm.normalWateringCans),
    "watering-can-gold": toNumber(farm.goldenWateringCans),
    "loot-acorn": toNumber(loot.acorn),
    "loot-broken-sword": toNumber(loot.brokenSword),
    "loot-normal-chest": toNumber(loot.normalChest),
    "loot-bear-claw": toNumber(loot.bearClaw),
    "loot-bear-hide": toNumber(loot.bearHide),
    "loot-dragon-horn": toNumber(loot.dragonHorn),
    "loot-gold-chest": toNumber(loot.goldChest)
  };
}

function applyPerriPetInventoryToSnapshot(snapshot, inventory) {
  const safeSnapshot = snapshot && typeof snapshot === "object" ? snapshot : {};
  const pet = safeSnapshot.pet && typeof safeSnapshot.pet === "object" ? safeSnapshot.pet : {};
  const farm = safeSnapshot.farm && typeof safeSnapshot.farm === "object" ? safeSnapshot.farm : {};
  const existingCapsules = Array.isArray(pet.capsules) ? pet.capsules : [];
  const capsuleQuantity = Math.max(toNumber(inventory["capsule-g"]), existingCapsules.filter((slot) => slot?.creature).length);
  const capsules = existingCapsules.slice(0, capsuleQuantity);
  while (capsules.length < capsuleQuantity) {
    capsules.push({ id: crypto.randomUUID(), creature: null });
  }
  const lootInventory = {
    acorn: toNumber(inventory["loot-acorn"]),
    brokenSword: toNumber(inventory["loot-broken-sword"]),
    normalChest: toNumber(inventory["loot-normal-chest"]),
    bearClaw: toNumber(inventory["loot-bear-claw"]),
    bearHide: toNumber(inventory["loot-bear-hide"]),
    dragonHorn: toNumber(inventory["loot-dragon-horn"]),
    goldChest: toNumber(inventory["loot-gold-chest"])
  };
  safeSnapshot.pet = {
    ...pet,
    normalFood: toNumber(inventory["food-normal"]),
    primeFood: toNumber(inventory["food-prime"]),
    greenEggs: toNumber(inventory["egg-green"]),
    capsules,
    lootInventory,
    combatItems: Object.values(lootInventory).reduce((total, quantity) => total + quantity, 0)
  };
  safeSnapshot.farm = {
    ...farm,
    seeds: toNumber(inventory["seed-green"]),
    redSeeds: toNumber(inventory["seed-red"]),
    purpleFruits: toNumber(inventory["fruit-purple"]),
    yellowFruits: toNumber(inventory["fruit-yellow"]),
    plotUnlockItems: toNumber(inventory["plot-unlock"]),
    normalWateringCans: toNumber(inventory["watering-can-normal"]),
    goldenWateringCans: toNumber(inventory["watering-can-gold"])
  };
  return safeSnapshot;
}

async function getPerriPetInventory(nick) {
  const rows = await fetchRows("shop_perripet_inventory", {
    filters: { nick: `eq.${normalizeNick(nick)}` },
    orderBy: "item_id.asc"
  });
  const inventory = emptyPerriPetInventory();
  rows.forEach((row) => {
    const itemId = String(row.item_id || "");
    if (itemId in inventory) inventory[itemId] = toNumber(row.quantity);
  });
  return { inventory, hasRows: rows.length > 0 };
}

async function savePerriPetInventory(nick, inventory) {
  for (const itemId of PERRIPET_ITEM_IDS) {
    await upsertRow("shop_perripet_inventory", {
      nick: normalizeNick(nick),
      item_id: itemId,
      quantity: Math.min(999999999, toNumber(inventory[itemId])),
      updated_at: nowIso()
    }, "nick,item_id", { returning: "minimal" });
  }
  return Object.fromEntries(PERRIPET_ITEM_IDS.map((itemId) => [itemId, toNumber(inventory[itemId])]));
}

async function savePerriPetStateRow(nick, snapshot) {
  await upsertRow("shop_perripet_states", {
    nick: normalizeNick(nick),
    version: Math.max(1, toNumber(snapshot?.version) || 1),
    pet_state: snapshot?.pet && typeof snapshot.pet === "object" ? snapshot.pet : {},
    farm_state: snapshot?.farm && typeof snapshot.farm === "object" ? snapshot.farm : {},
    encounters_state: Array.isArray(snapshot?.encounters) ? snapshot.encounters : [],
    updated_at: nowIso()
  }, "nick", { returning: "minimal" });
}

async function getPerriPetRpgInventory(nick) {
  const rows = await fetchRows("shop_rpg_inventory", {
    filters: { nick: `eq.${normalizeNick(nick)}`, quantity: "gt.0" },
    orderBy: "item_id.asc"
  });
  const byCatalogId = Object.fromEntries(rows.map((row) => [String(row.item_id || ""), Math.max(0, toNumber(row.quantity))]));
  return Object.fromEntries(
    Object.entries(PERRIPET_RPG_ITEM_IDS).map(([perriPetId, catalogId]) => [perriPetId, byCatalogId[catalogId] || 0])
  );
}
...
async function publicShopPerriPetState(sessionToken) {
  ...
  const [storedState, inventory, rpgInventory, storedPerriPetInventory] = await Promise.all([
    ...
    getPerriPetRpgInventory(sessionResult.nick),
    getPerriPetInventory(sessionResult.nick)
  ]);
  let perriPetInventory = storedPerriPetInventory.inventory;
  if (!storedPerriPetInventory.hasRows && storedState.snapshot) {
    perriPetInventory = await savePerriPetInventory(sessionResult.nick, perriPetInventoryFromSnapshot(storedState.snapshot));
  }
  if (storedState.legacy) {
    inventory.chatarra = Math.max(toNumber(inventory.chatarra), toNumber(storedState.snapshot?.pet?.scrap));
...
async function publicShopPerriPetSave(sessionToken, snapshot) {
  ...
  try {
    ...
    if (encoded.length > 750000) return { ok: false, error: "perripet_state_too_large" };
    safeSnapshot = JSON.parse(encoded);
  } catch {
    return { ok: false, error: "invalid_perripet_state" };
  }
  ...
  if (previousPet) {
    const gemDelta = Math.trunc(toNumber(incomingPet.coins) - toNumber(previousPet.coins));
    const pcDelta = Math.trunc(toNumber(incomingPet.perriCoins) - toNumber(previousPet.perriCoins));
    const scrapDelta = Math.trunc(toNumber(incomingPet.scrap) - toNumber(previousPet.scrap));
    ...
    const previousPerriPetInventory = perriPetInventoryFromSnapshot(previousSnapshot);
    ...
    const previousRpg = previousPet.rpgInventory && typeof previousPet.rpgInventory === "object" ? previousPet.rpgInventory : {};
    ...
    for (const [perriPetId, catalogId] of Object.entries(PERRIPET_RPG_ITEM_IDS)) {
      ...
      await upsertRow("shop_rpg_inventory", {
        nick: sessionResult.nick,
        item_id: catalogId,
        quantity: Math.min(9999, toNumber(currentRpg[perriPetId]) + increase),
        updated_at: nowIso()
      }, "nick,item_id", { returning: "minimal" });
    }
  }

  const savedInventory = previousPet ? await saveInventory(sessionResult.nick, inventory) : inventory;
  ...
  safeSnapshot.savedAt = Date.now();

  await savePerriPetStateRow(sessionResult.nick, safeSnapshot);
...
async function publicShopRpgPurchase(sessionToken, itemId, quantity) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) return sessionResult;
  const requestedQuantity = [1, 5, 30].includes(Number(quantity)) ? Number(quantity) : 1;
  const result = await supabaseRequest("rpc/shop_rpg_purchase_items", {
    method: "POST",
    body: JSON.stringify({ p_nick: sessionResult.nick, p_item_id: String(itemId || "").trim(), p_quantity: requestedQuantity })
  });
  if (!result?.ok) return result || { ok: false, error: "rpg_purchase_failed" };
  return { ok: true, purchase: result, ...(await buildRpgState(sessionResult.nick)) };
}

async function publicShopRpgEquipArmor(sessionToken, slot, itemId) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) return sessionResult;
  const result = await supabaseRequest("rpc/shop_rpg_equip_armor", {
    method: "POST",
    body: JSON.stringify({
      p_nick: sessionResult.nick,
      p_slot: String(slot || "").trim(),
      p_item_id: String(itemId || "").trim()
    })
  });
  if (!result?.ok) return result || { ok: false, error: "rpg_equip_failed" };
  return { ok: true, equipment: result, ...(await buildRpgState(sessionResult.nick)) };
}
...
async function publicShopRpgSetLoadout(sessionToken, itemId, enabled) {
  ...
  if (!result?.ok) return result || { ok: false, error: "rpg_loadout_failed" };
  return { ok: true, loadoutChange: result, ...(await buildRpgState(sessionResult.nick)) };
}
...
async function publicShopRpgSaveCombat(sessionToken, combat) {
  ...
  const row = {
    nick: sessionResult.nick,
    round_number: Math.floor(boundedNumber(safeCombat.round, 1, 10000, 1)),
    player_hp: boundedNumber(safeCombat.playerHp, -100000, 1000000, 0),
    player_max_hp: boundedNumber(safeCombat.playerMaxHp, 1, 1000000, 100),
    enemy_hp: boundedNumber(safeCombat.enemyHp, 0, 1000000, 5000),
    enemy_max_hp: boundedNumber(safeCombat.enemyMaxHp, 1, 1000000, 5000),
    is_undead: Boolean(safeCombat.undead),
    player_statuses: Array.isArray(safeCombat.playerStatuses) ? safeCombat.playerStatuses.slice(0, 20) : [],
    enemy_statuses: Array.isArray(safeCombat.enemyStatuses) ? safeCombat.enemyStatuses.slice(0, 20) : [],
  ...
  const bossReward = await supabaseRequest("rpc/shop_rpg_claim_boss_reward", {
    method: "POST",
    body: JSON.stringify({ p_nick: sessionResult.nick })
  });
  await appendActivity(sessionResult.nick, "PerriRPG", "Rey Cangrosio IV derrotado", `Victoria en la ronda ${row.round_number}`);
  const inventory = await getInventory(sessionResult.nick);
  const profile = await getProfile(sessionResult.nick);
  const achievementResult = await syncAchievementSystem(sessionResult.nick, inventory, profile);
  await touchPublicUser(sessionResult.nick, inventory, achievementResult.profile);
  return {
    ok: true,
    bossDefeated: true,
    bossReward,
...
async function publicShopSetTitle(sessionToken, achievementId) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) return sessionResult;
  const normalizedAchievementId = String(achievementId || "").trim();
  if (normalizedAchievementId && !ACHIEVEMENT_ID_SET.has(normalizedAchievementId)) {
    return { ok: false, error: "invalid_title" };
  }
  return withNickLock(sessionResult.nick, async () => {
    const inventory = await getInventory(sessionResult.nick);
    let profile = await getProfile(sessionResult.nick);
    const achievementResult = await syncAchievementSystem(sessionResult.nick, inventory, profile);
    profile = achievementResult.profile;
    if (normalizedAchievementId && !achievementResult.achievementSystem.unlockedIds.includes(normalizedAchievementId)) {
      return { ok: false, error: "title_locked", profile, achievementSystem: achievementResult.achievementSystem };
    }
    const savedProfile = await saveProfile(sessionResult.nick, { ...profile, selectedTitleId: normalizedAchievementId });
    await touchPublicUser(sessionResult.nick, inventory, savedProfile);
    return {
      ok: true,
      profile: savedProfile,
      achievementSystem: { ...achievementResult.achievementSystem, selectedTitleId: normalizedAchievementId }
    };
  });
}
...
async function publicShopPurchase(sessionToken, itemKey, amount) {
  ...
  return withNickLock(sessionResult.nick, async () => {
    ...
    const currencyLabel = String(item.currencyLabel || (currencyKey === "polvoGema" ? "Gemas" : "PC"));
    ...
    if (normalizedItemKey !== "llaves" && !item.maxPerUser) {
      normalizedAmount = 1;
    ...
    if (item.maxPerUser) {
      normalizedAmount = 1;
      const ownedAmount = Math.max(0, toNumber(inventory[normalizedItemKey]));
      if (ownedAmount >= item.maxPerUser) {
        return {
          ok: false,
...
async function publicShopUpgradeEquipment(sessionToken, gearType, usePerricita) {
  ...
  return withNickLock(sessionResult.nick, async () => {
    ...
    await appendActivity(
      ...
      `${currentStage.currentLabel} -> ${currentStage.nextLabel} | Coste: ${currentStage.cost} Gemas | Exito: ${effectiveChance}%${usePerricita ? " | Perricita: si" : " | Perricita: no"}`
    );
...
function publicPerriCasillasGame(game) {
  const windowStartedMs = Date.parse(String(game.windowStartedAt || ""));
  const storedPlays = Math.max(0, Math.min(PERRI_CASILLAS_MAX_GAMES, Number(game.playsInWindow || 0)));
  const windowActive = storedPlays >= PERRI_CASILLAS_MAX_GAMES && Number.isFinite(windowStartedMs) && Date.now() - windowStartedMs < PERRI_CASILLAS_WINDOW_MS;
  const playsUsed = storedPlays >= PERRI_CASILLAS_MAX_GAMES ? (windowActive ? storedPlays : 0) : storedPlays;
  return {
    roundId: String(game.roundId || ""),
    playsUsed,
    playsRemaining: Math.max(0, PERRI_CASILLAS_MAX_GAMES - playsUsed),
    cooldownUntil: playsUsed >= PERRI_CASILLAS_MAX_GAMES
      ? new Date(windowStartedMs + PERRI_CASILLAS_WINDOW_MS).toISOString()
      : "",
    cells: game.cells.map((cell) => cell.opened
      ? {
          opened: true,
          reward: {
            label: cell.reward.label,
            kind: cell.reward.kind,
            amount: cell.reward.amount,
            image: cell.reward.image
          }
        }
      : { opened: false, reward: null })
  };
}

async function getOrCreatePerriCasillasGame(nick) {
  const row = await fetchRow("perri_casillas_games", {
    filters: { nick: `eq.${normalizeNick(nick)}` }
  });
  const storedCells = row && Array.isArray(row.cells) ? row.cells : [];
  if (row) {
    const storedGame = {
      roundId: String(row.round_id || ""),
      cells: storedCells,
      boardVersion: Math.max(0, Number(row.board_version || 0)),
      playsInWindow: Math.max(0, Number(row.plays_in_window || 0)),
      windowStartedAt: String(row.window_started_at || "")
    };
    const originalPlaysInWindow = storedGame.playsInWindow;
    const originalWindowStartedAt = storedGame.windowStartedAt;
    const needsBoardReset = storedCells.length !== PERRI_CASILLAS_TOTAL || storedGame.boardVersion !== PERRI_CASILLAS_BOARD_VERSION;
    const game = !needsBoardReset
      ? storedGame
      : {
          ...storedGame,
          roundId: crypto.randomUUID(),
          cells: createPerriCasillasBoard(),
          boardVersion: PERRI_CASILLAS_BOARD_VERSION,
          playsInWindow: 0,
          windowStartedAt: ""
        };
    const windowStartedMs = Date.parse(game.windowStartedAt);
    if (game.playsInWindow >= PERRI_CASILLAS_MAX_GAMES && (!Number.isFinite(windowStartedMs) || Date.now() - windowStartedMs >= PERRI_CASILLAS_WINDOW_MS)) {
      game.playsInWindow = 0;
      game.windowStartedAt = "";
    } else if (game.playsInWindow < PERRI_CASILLAS_MAX_GAMES) {
      game.windowStartedAt = "";
    }
    if (needsBoardReset || game.playsInWindow !== originalPlaysInWindow || game.windowStartedAt !== originalWindowStartedAt) {
      await savePerriCasillasGame(nick, game);
    }
    return game;
  }
  ...
  await upsertRow("perri_casillas_games", {
    nick: normalizeNick(nick),
    round_id: game.roundId,
    cells: game.cells,
    board_version: PERRI_CASILLAS_BOARD_VERSION,
    plays_in_window: game.playsInWindow,
    window_started_at: game.windowStartedAt || null,
    updated_at: nowIso()
  }, "nick", { returning: "minimal" });
  return game;
}

async function savePerriCasillasGame(nick, game) {
  await upsertRow("perri_casillas_games", {
    nick: normalizeNick(nick),
    round_id: game.roundId,
    cells: game.cells,
    board_version: PERRI_CASILLAS_BOARD_VERSION,
    plays_in_window: Math.max(0, Number(game.playsInWindow || 0)),
    window_started_at: game.windowStartedAt || null,
    updated_at: nowIso()
  }, "nick", { returning: "minimal" });
}

async function publicShopGetPerriCasillas(sessionToken) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) return sessionResult;
...
async function publicShopOpenPerriCasilla(sessionToken, openId, cellIndex) {
  ...
  if (!Number.isInteger(normalizedCellIndex) || normalizedCellIndex < 0 || normalizedCellIndex >= PERRI_CASILLAS_TOTAL) {
    return {
      ok: false,
      error: "invalid_cell",
      nick: sessionResult.nick,
  ...
  return withNickLock(sessionResult.nick, async () => runIdempotentOperation(
    normalizedOpenId,
    "publicShopOpenPerriCasilla",
    sessionResult.nick,
    async () => {
      const inventory = await getInventory(sessionResult.nick);
      const game = await getOrCreatePerriCasillasGame(sessionResult.nick);
      const cell = game.cells[normalizedCellIndex];
      if (cell.opened) {
        return {
          ok: true,
          nick: sessionResult.nick,
      ...
      if (game.playsInWindow >= PERRI_CASILLAS_MAX_GAMES) {
        const gameForClient = publicPerriCasillasGame(game);
        return {
          ok: false,
          error: "perri_casillas_cooldown",
          nick: sessionResult.nick,
          cost: PERRI_CASILLAS_COST,
          cooldownUntil: gameForClient.cooldownUntil,
          game: gameForClient,
          inventory
        };
      }
      if (toNumber(inventory.polvoGema) < PERRI_CASILLAS_COST) {
        return {
          ok: false,
          error: "not_enough_polvoGema",
          nick: sessionResult.nick,
      ...
      let profile = await getProfile(sessionResult.nick);
      if (reward.kind === "logoRandom") {
        const logoCatalog = await listLogoCatalog();
        const unknownLogos = listUnknownLogos(profile, logoCatalog);
        const availableLogos = unknownLogos.length ? unknownLogos : logoCatalog;
        const selectedLogo = availableLogos[randomInt(0, Math.max(0, availableLogos.length - 1))] || null;
        if (selectedLogo) {
          reward.label = `Perrilogo: ${selectedLogo.name}`;
          reward.image = selectedLogo.src;
          reward.logoId = selectedLogo.id;
          profile = await saveProfile(sessionResult.nick, {
            ...profile,
            ownedLogoIds: profile.ownedLogoIds.includes(selectedLogo.id)
              ? profile.ownedLogoIds.slice()
              : [...profile.ownedLogoIds, selectedLogo.id]
          });
        }
      }
      cell.reward = reward;
      const inventoryDelta = mergeInventoryDelta({ polvoGema: -PERRI_CASILLAS_COST }, reward.inventoryDelta || {});
      const savedInventory = await saveInventory(sessionResult.nick, applyInventoryDelta(inventory, inventoryDelta));
      cell.opened = true;
      game.playsInWindow += 1;
      if (game.playsInWindow >= PERRI_CASILLAS_MAX_GAMES) {
        game.windowStartedAt = nowIso();
      }
      await savePerriCasillasGame(sessionResult.nick, game);
      await touchPublicUser(sessionResult.nick, savedInventory, profile);
      await appendActivity(
        ...
        "PerriCasillas",
        reward.label,
        `Coste: ${PERRI_CASILLAS_COST} Gemas | Premio: ${reward.label}`
      );
...
async function publicShopPlayCard(sessionToken, reward, cardNumber, forced, playId) {
  ...
  return withNickLock(sessionResult.nick, async () => runIdempotentOperation(playId, "publicShopPlayCard", sessionResult.nick, async () => {
    ...
    const useFreePlay = !forced && Boolean(dailyFreePlays.cardsAvailable);
    if (!rewardDelta) {
      return {
        ok: false,
    ...
    if (!forced && !useFreePlay && toNumber(inventory.monedas) <= 0) {
      return {
        ok: false,
        error: "no_monedas",
        nick: sessionResult.nick,
    ...
    if (useFreePlay) {
      const freePlayResult = await consumeDailyFreePlay(sessionResult.nick, "cards");
      dailyFreePlays = freePlayResult.state;
    }

    inventory = applyInventoryDelta(inventory, mergeInventoryDelta((forced || useFreePlay) ? {} : { monedas: -1 }, rewardDelta));
    const savedInventory = await saveInventory(sessionResult.nick, inventory);
    const profile = await getProfile(sessionResult.nick);
    await touchPublicUser(sessionResult.nick, savedInventory, profile);
    await appendActivity(sessionResult.nick, "Cartas", String(reward || "").trim(), `Carta ${Number(cardNumber || 0) || "?"}${useFreePlay ? " | Tirada gratis diaria: si" : ""}`);
...
async function publicShopOpenChest(sessionToken, chestOpenId) {
  ...
  return withNickLock(sessionResult.nick, async () => runIdempotentOperation(chestOpenId, "publicShopOpenChest", sessionResult.nick, async () => {
    ...
    let nextInventory = applyInventoryDelta(inventory, rolledChest.reward.inventoryDelta);
...
async function publicShopOpenInventoryChest(sessionToken, chestOpenId, chestType) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) {
    return sessionResult;
  }
  ...
  return withNickLock(sessionResult.nick, async () => runIdempotentOperation(chestOpenId, "publicShopOpenInventoryChest", sessionResult.nick, async () => {
    const normalizedChestType = String(chestType || "perriCofres").trim() || "perriCofres";
    const isMinigameChest = normalizedChestType === "perriCofresMinijuego";
    const chestInventoryKey = isMinigameChest ? "perriCofresMinijuego" : "perriCofres";
    const noChestError = isMinigameChest ? "no_minigame_chests" : "no_perri_chests";
    const inventory = await getInventory(sessionResult.nick);
    if (toNumber(inventory[chestInventoryKey]) <= 0) {
      return {
        ok: false,
        error: noChestError,
        nick: sessionResult.nick,
    ...
    const consumeResult = await consumeNextInventoryChestRow(sessionResult.nick, chestInventoryKey);
    if (consumeResult.tracked && !consumeResult.consumed) {
      const syncedInventory = (await syncInventoryChestState(sessionResult.nick, inventory)).inventory;
      return {
        ok: false,
        error: noChestError,
        nick: sessionResult.nick,
    ...
    const openingInventory = consumeResult.tracked
      ? (await syncInventoryChestState(sessionResult.nick, inventory)).inventory
      : inventory;
    const logoCatalog = await listLogoCatalog();
    const knownLogoIds = new Set(logoCatalog.map((entry) => entry.id));
    const profile = await getProfile(sessionResult.nick);
    const rewardBundle = isMinigameChest ? buildMinigameChestRewards() : buildPerriChestRewards(profile, logoCatalog);
    const nextInventory = applyInventoryDelta(
      openingInventory,
      mergeInventoryDelta(consumeResult.tracked ? {} : { [chestInventoryKey]: -1 }, getPerriChestInventoryDelta(rewardBundle))
    );
    const nextProfile = isMinigameChest ? profile : applyPerriChestProfileRewards(profile, rewardBundle, knownLogoIds);
    const savedInventory = await saveInventory(sessionResult.nick, nextInventory);
    const savedProfile = await saveProfile(sessionResult.nick, nextProfile);
    await touchPublicUser(sessionResult.nick, savedInventory, savedProfile);
    await updateRows("shop_access", {
    ...
    await appendActivity(
      sessionResult.nick,
      isMinigameChest ? "Perricofre minijuego" : "Perricofre",
...
function createDuelTargetToken(attackerNick, defenderNick) {
...
async function selectRandomDuelOpponent(attackerNick) {
  ...
  const attackerKey = normalizeNickKey(attackerNick);
  const candidates = accessRows.map((row) => normalizeNick(row.nick)).filter((nick) => nick && normalizeNickKey(nick) !== attackerKey);
  return candidates.length ? candidates[crypto.randomInt(candidates.length)] : "";
}

async function publicShopGetDuelTarget(sessionToken) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) {
    return sessionResult;
  }
...
async function publicShopClaimDailyPass(sessionToken, claimId) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) {
    return sessionResult;
  }
  ...
  return withNickLock(sessionResult.nick, async () => runIdempotentOperation(claimId, "publicShopClaimDailyPass", sessionResult.nick, async () => {
    const currentState = await getDailyPassState(sessionResult.nick);
    if (currentState.notStarted) {
      return {
        ok: false,
        error: "daily_pass_not_started",
        nick: sessionResult.nick,
        dailyPass: currentState
      };
    }
    if (currentState.expired) {
      return {
        ok: false,
        error: "daily_pass_expired",
        nick: sessionResult.nick,
        dailyPass: currentState
      };
    }
    ...
    if (!currentState.canClaimToday) {
      return {
        ok: false,
        error: "daily_pass_already_claimed_today",
        nick: sessionResult.nick,
        dailyPass: currentState
      };
    }

    const rewardDefinition = getDailyPassRewardDefinition(currentState.currentRewardDay);
    if (!rewardDefinition) {
      return {
        ok: false,
        error: "daily_pass_reward_missing",
        nick: sessionResult.nick,
        dailyPass: currentState
      };
    }
...
async function adminActivatePremium(nick) {
  ...
  return withNickLock(normalizedNick, async () => {
    ...
    const profile = await getProfile(normalizedNick);
    await touchPublicUser(normalizedNick, savedInventory, profile);
    await appendActivity(normalizedNick, "Ruleta", "Activacion Premium", "Coste: 500 Gemas | Tirada: 1");
    return {
      ok: true,
      nick: normalizedNick,
      inventory: savedInventory
...
async function adminPerformAttack(attackerNick, defenderNick, weapon, shieldType, outcome, force) {
  ...
  return withAttackLock(normalizedAttackerNick, normalizedDefenderNick, async () => {
    ...
    if (normalizedOutcome === "Impacto") {
      ...
      duelRewards = [
        {
          kind: "perriCofresMinijuego",
          label: "Perricofre de minijuego",
          amount: 1,
          image: "./assets/perricofres-minijuego-cerrado.png"
        }
      ];
      if (transferredPc > 0) {
        duelRewards.push({
    ...
    return {
      ...
      transferredPc,
      minigameChestGranted: normalizedOutcome === "Impacto",
      duelRewards,
...
async function handleShopAction(payload) {
  ...
  switch (action) {
    ...
    case "publicShopRpgState":
      return publicShopRpgState(payload.sessionToken);
    case "publicShopPerriPetState":
    ...
    case "publicShopRpgEquipArmor":
      return publicShopRpgEquipArmor(payload.sessionToken, payload.slot, payload.itemId);
    case "publicShopRpgSetLoadout":
      return publicShopRpgSetLoadout(payload.sessionToken, payload.itemId, payload.enabled);
    case "publicShopRpgConsume":
      return publicShopRpgConsume(payload.sessionToken, payload.itemId);
    case "publicShopRpgSaveCombat":
      return publicShopRpgSaveCombat(payload.sessionToken, payload.combat);
    case "publicShopRpgClearCombat":
    ...
    case "publicShopRequestAccess":
      return publicShopRequestAccess(payload.nick, payload.password, payload.requestMessage);
    case "publicShopListAccessRequestsPanel":
    ...
    case "publicShopSetTitle":
      return publicShopSetTitle(payload.sessionToken, payload.achievementId);
    ...
    case "adminListAccessRequests":
      return adminListAccessRequests(payload.limit, payload.status);
    case "adminResolveAccessRequest":
      return adminResolveAccessRequest(payload.requestId, payload.decision, payload.adminNote);
    ...
    case "publicShopGetPerriCasillas":
      return publicShopGetPerriCasillas(payload.sessionToken);
    case "publicShopResetPerriCasillas":
      return publicShopResetPerriCasillas(payload.sessionToken);
    case "publicShopOpenPerriCasilla":
      return publicShopOpenPerriCasilla(payload.sessionToken, payload.openId || payload.operationId, payload.cellIndex);
    case "publicShopCraftKey":
      return publicShopCraftKey(payload.sessionToken);
    case "publicShopClaimDailyPass":
      return publicShopClaimDailyPass(payload.sessionToken, payload.claimId);
    ...
    case "publicShopPerformAttack":
      ...
      case "publicShopLeaveChat":
        return publicShopLeaveChat(payload.sessionToken);
      case "publicShopToggleChatLike":
        return publicShopToggleChatLike(payload.sessionToken, payload.messageId);
    case "publicShopToggleChatReaction":
      return publicShopToggleChatReaction(payload.sessionToken, payload.messageId, payload.reactionKey);
