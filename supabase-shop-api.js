...
async function savePerriPetInventory(nick, inventory) {
  const rows = PERRIPET_ITEM_IDS.map((itemId) => ({
      nick: normalizeNick(nick),
      item_id: itemId,
      quantity: Math.min(999999999, toNumber(inventory[itemId])),
      updated_at: nowIso()
    }));
  await upsertRow("shop_perripet_inventory", rows, "nick,item_id", { returning: "minimal" });
...
async function publicShopPerriPetSave(sessionToken, snapshot) {
  ...
  if (basePet) {
    ...
    const previousRpg = syncBase?.rpgInventory && typeof syncBase.rpgInventory === "object"
      ? syncBase.rpgInventory
...
async function publicShopOpenPerriCasilla(sessionToken, openId, cellIndex) {
  ...
  return withNickLock(sessionResult.nick, async () => runIdempotentOperation(
    ...
    async () => {
      ...
      if (toNumber(inventory.monedas) < PERRI_CASILLAS_COST) {
        return {
          ok: false,
          error: "no_monedas",
          nick: sessionResult.nick,
      ...
      cell.reward = reward;
      const inventoryDelta = mergeInventoryDelta({ monedas: -PERRI_CASILLAS_COST }, reward.inventoryDelta || {});
      ...
      await appendActivity(
        ...
        "PerriCasillas",
        reward.label,
        `Coste: ${PERRI_CASILLAS_COST} moneda | Premio: ${reward.label}`
      );
