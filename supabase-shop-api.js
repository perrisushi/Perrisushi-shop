...
const PERRI_CASILLAS_BOARD_VERSION = 3;
...
const PERRI_CASILLAS_REWARDS = [
  ...
  { count: 5, label: "100 PC", kind: "pc", amount: 100, image: "./assets/moneda.png", inventoryDelta: { pc: 100 } },
  { count: 5, label: "250 PC", kind: "pc", amount: 250, image: "./assets/moneda.png", inventoryDelta: { pc: 250 } },
  ...
  { count: 3, label: "200 PG", kind: "polvoGema", amount: 200, image: "./assets/polvo-de-gema.png", inventoryDelta: { polvoGema: 200 } },
  { count: 2, label: "500 PG", kind: "polvoGema", amount: 500, image: "./assets/polvo-de-gema.png", inventoryDelta: { polvoGema: 500 } },
  { count: 1, label: "Logo Aleatorio", kind: "logoRandom", amount: 1, image: "./assets/marco-logo.png", inventoryDelta: {} },
  { count: 1, label: "3000 PC", kind: "pc", amount: 3000, image: "./assets/moneda.png", inventoryDelta: { pc: 3000 } },
  ...
  { count: 3, label: "500 PC", kind: "pc", amount: 500, image: "./assets/moneda.png", inventoryDelta: { pc: 500 } }
];
...
function mapDailyPassRewardItemForClient(item) {
  ...
  switch (normalizedType) {
    ...
    case "polvoGema":
      return { type: "polvoGema", qty, label: "PG", icon: "./assets/polvo-de-gema.png" };
    case "perricita":
...
function buildCardRewardDelta(reward) {
  ...
  if (Object.prototype.hasOwnProperty.call(pcRewards, normalizedReward)) {
    return { pc: pcRewards[normalizedReward] };
  }
...
async function publicShopRpgPurchase(sessionToken, itemId) {
  const sessionResult = await requireSession(sessionToken);
  if (!sessionResult.ok) return sessionResult;
  const result = await supabaseRequest("rpc/shop_rpg_purchase_item", {
    method: "POST",
    body: JSON.stringify({ p_nick: sessionResult.nick, p_item_id: String(itemId || "").trim() })
  });
...
async function getOrCreatePerriCasillasGame(nick) {
  ...
  if (row) {
    ...
    const needsBoardReset = storedCells.length !== 36 || storedGame.boardVersion !== PERRI_CASILLAS_BOARD_VERSION;
...
async function publicShopResetPerriCasillas(sessionToken) {
  ...
  return withNickLock(sessionResult.nick, async () => {
    ...
    await appendActivity(sessionResult.nick, "PerriCasillas", "Nueva partida", "Tablero de 36 casillas reiniciado");
    return {
      ok: true,
      nick: sessionResult.nick,
      cost: PERRI_CASILLAS_COST,
...
async function publicShopOpenPerriCasilla(sessionToken, openId, cellIndex) {
  ...
  if (!Number.isInteger(normalizedCellIndex) || normalizedCellIndex < 0 || normalizedCellIndex >= 36) {
    return {
      ok: false,
      error: "invalid_cell",
      nick: sessionResult.nick,
  ...
  return withNickLock(sessionResult.nick, async () => runIdempotentOperation(
    ...
    async () => {
      ...
      await appendActivity(
        ...
        "PerriCasillas",
        reward.label,
        `Coste: ${PERRI_CASILLAS_COST} PG | Premio: ${reward.label}`
      );
...
async function handleShopAction(payload) {
  ...
  switch (action) {
    ...
    case "publicShopRpgPurchase":
      return publicShopRpgPurchase(payload.sessionToken, payload.itemId);
