...
const BUILT_IN_ADMIN_NICKS = new Set(["perrisushi"]);
...
function isAccessRowAdmin(accessRow) {
  const normalizedNick = normalizeNick(accessRow && accessRow.nick ? accessRow.nick : "");
  if (!normalizedNick) {
    return false;
  }

  if (accessRow && accessRow.is_admin === true) {
    return true;
  }

  return BUILT_IN_ADMIN_NICKS.has(normalizedNick.toLowerCase());
}
...
async function requireSession(sessionToken) {
  ...
  return {
    ...
    isAdmin: isAccessRowAdmin(accessRow)
  };
}
...
async function buildShopState(nick) {
  const accessRow = await fetchNickRowCaseInsensitive("shop_access", nick);
...
async function adminUpsertWebAccess(nick, password, active) {
  ...
  return {
    ...
    nick: normalizedNick,
    active: activeValue,
    isAdmin: isAccessRowAdmin({ nick: normalizedNick })
  };
}
