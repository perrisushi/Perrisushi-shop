...
async function appendRandomKeyTicket(nick, obtainedAtIso) {
  ...
  await appendNotification(
    nick,
    NOTIFICATION_KINDS.ticket,
    "Ticket recibido",
    "Has recibido 1 Ticket de Random Key.",
    { amount: 1 }
  );
}
...
async function publicShopRedeemPerriChest(sessionToken, chestToken, redeemId) {
  ...
  return withNickLock(sessionResult.nick, async () => runIdempotentOperation(redeemId, "publicShopRedeemPerriChest", sessionResult.nick, async () => {
    ...
    const fragmentReward = (Array.isArray(rewardBundle.rewards) ? rewardBundle.rewards : []).find((reward) => String(reward?.kind || "").trim() === "fragmentoKey");
    if (fragmentReward && toNumber(fragmentReward.quantity) > 0) {
      await appendNotification(
        sessionResult.nick,
        NOTIFICATION_KINDS.fragmento,
        "Fragmento conseguido",
        `Has conseguido ${formatRewardLabel(toNumber(fragmentReward.quantity), "Fragmento de Key")}.`,
        { amount: toNumber(fragmentReward.quantity) }
      );
    }
